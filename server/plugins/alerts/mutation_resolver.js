...
const addAlertResolver = async (root, { id, symbol, low_price }, context) => {
  try {
    ...
    return await context.pool.query(`INSERT INTO stock.alerts(id, symbol, low_price) VALUES ('${id}', '${symbol}', '${low_price}') ON CONFLICT DO NOTHING; SELECT * FROM stock.alerts WHERE id = '${id}'`).then(res => res[1].rows);
  } catch (error) {
...
const removeAlertResolver = async (root, { alert_id, id }, context) => {
  try {
    ...
    return await context.pool.query(`DELETE FROM stock.alerts WHERE alert_id = '${alert_id}'; SELECT * FROM stock.alerts WHERE id = '${id}'`).then(res => res[1].rows);
  } catch (error) {
