...
const addWatchResolver = async (root, { id, symbol }, context) => {
  try {
    ...
    return await context.pool
      .query(
        `INSERT INTO stock.watchlist(id, symbol) VALUES ('${id}', '${symbol}') ON CONFLICT DO NOTHING; SELECT * FROM stock.watchlist WHERE id = '${id}'`
      )
      .then(res => res[1].rows);
  } catch (error) {
...
const removeWatchResolver = async (root, { id, symbol }, context) => {
  try {
    ...
    return await context.pool
      .query(
        `DELETE FROM stock.watchlist WHERE id = '${id}' and symbol = '${symbol}'; SELECT * FROM stock.watchlist WHERE id = '${id}'`
      )
      .then(res => res[1].rows);
  } catch (error) {
