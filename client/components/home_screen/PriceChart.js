...
import {
  GET_STOCK_INTRADAY_PRICE,
  GET_STOCK_INTRADAY_H_PRICE,
  GET_STOCK_DAILY_PRICE
} from "../queries";
...
export default function PriceChart(props) {
  ...
  const { loading, error, data } = useQuery(
    RANGE_QUERY_MAPPING[props.range][0],
    {
  ...
  function findTickDateList(prices) {
    ...
    for (
      let i = prices.length - RANGE_QUERY_MAPPING[props.range][2];
      i < prices.length;
      i++
    ) {
  ...
  if (
    !R.isNil(data.security.stock_price[RANGE_QUERY_MAPPING[props.range][1]])
  ) {
    prices =
      data.security.stock_price[RANGE_QUERY_MAPPING[props.range][1]].prices;
  } else {
    return <Text>Loading ...</Text>;
  }
        ...
        >
          <VictoryAxis
            ...
            style={{
              ...
              grid: {
                stroke: x => {
                  if (x.text != undefined) return "grey";
                },
