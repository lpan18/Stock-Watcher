...
export default function Search(props) {
  ...
  const updateSearchTxt = (value) => {
    setSearchTxt(value);
    if (!value) {
      setMatchedSymbols([]);
    } else if (value.length <= maxSymbolLength) {
      setMatchedSymbols(getMatchedSymbols(SYMBOLS, value.toUpperCase()).slice(0, 4)); // slice to 2 to reduce api call, for dev purpose
    }
  };
  ...
  return (
    <View>
      <SearchBar
        ...
        showCancel
      />
      {isMatchedLoading || matchedData.length == 0 ? (
        <Text />
      ) : (
