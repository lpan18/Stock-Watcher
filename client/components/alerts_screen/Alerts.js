...
const Alerts = (props) => {
  ...
  const handleRemove = async (alert_id, id) => {
    const response = await removeAlertMut({ variables: { alert_id: alert_id, id: id } });
  ...
  return (
    <View style={styles.container}>
      <NavigationEvents
        onWillFocus={() => GetAlert.refetch()}
      />
      {alertData.length == 0 ? <Text /> : (
        <View>
          <View style={{ flexDirection: 'row', justifyContent: 'space-around', alignItems: "center", height: 50 }}>
            <Text style={styles.header}>Symbol
                    </Text>
            <Text style={styles.header}>Alert Price
                    </Text>
            <Text style={styles.header}>Created Date
                    </Text>
          </View>
          {alertData.map(d => (
            <Swipeable key={d.alert_id} rightButtons={[<Text style={styles.remove} onPress={() => handleRemove(d.alert_id, user.id)}>Remove</Text>]}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-around', alignItems: "center", height: 40 }}>
                <Text style={styles.rowText}>{d.symbol}
                </Text>
                <Text style={styles.rowText}>{d.low_price}
                </Text>
                <Text style={styles.rowText}>{d.sys_create_time.substr(0, 10)}
                </Text>
              </View>
            </Swipeable>
          ))}
